...
class KubeJobReplicaCost(KubeJobProgress):
    ...
    def calculate_error(self):
        rep = self._get_num_replicas() or self.last_replicas
        replica_price = self.get_current_cost()
        replica_cost = rep * replica_price
        err = replica_cost - self.desired_cost

        self.pretty_print(rep, replica_price, err)
        self.last_error = err
        self.last_cost = replica_cost
        self.last_rep = rep
        return err

    def pretty_print(self, replica_number, replica_price, error):
        self.LOG.log('Replica number: {}\nReplica price: {}\n'
                     'Error: {}'.format(replica_number, replica_price,
                                        error))
    ...
    def get_current_cost(self):
        return float(json.loads(requests.get(self.cluster_info_url.strip()).text))

    def get_dimensions(self):
        return {'application_id': self.app_id,
                'service': 'kubejobs_replica_cost'}


PLUGIN = KubeJobReplicaCost
